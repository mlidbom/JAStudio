...
`csharp-ls` doesn't watch the solution graph. **Tell the user to restart Claude Code after any edit to a `.csproj` or `.slnx` file** (or after `git checkout` / `git submodule update` if either of those moves). `.cs` file changes — including new files, deletes, and renames — are picked up live without restart.
